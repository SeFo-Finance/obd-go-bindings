	...
	customTypeGen := map[MessageType]func([]reflect.Value, *rand.Rand){
		...
		MsgOpenChannel: func(v []reflect.Value, r *rand.Rand) {
			req := OpenChannel{
				...
				MaxValueInFlight: UnitPrec11(MilliSatoshi(r.Int63())),
				ChannelReserve:   UnitPrec8(btcutil.Amount(r.Int63())),
				HtlcMinimum:      UnitPrec11(MilliSatoshi(r.Int31())),
		...
		MsgChannelUpdate: func(v []reflect.Value, r *rand.Rand) {
			...
			req := ChannelUpdate{
				...
				HtlcMinimumMsat: UnitPrec11(MilliSatoshi(r.Int63())),
				HtlcMaximumMsat: UnitPrec11(maxHtlc),
				BaseFee:         uint32(r.Int31()),
				FeeRate:         UnitPrec11(uint32(r.Int31())),
				ExtraOpaqueData: make([]byte, 0),
			}
			req.Signature, err = NewSigFromSignature(testSig)
			if err != nil {
