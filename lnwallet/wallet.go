...
import (
	...
	"github.com/SeFo-Finance/obd-go-bindings/lnwallet/chanvalidate"
...
func (l *LightningWallet) ListUnspentWitnessFromDefaultAccount(
	...
	return l.WalletController.ListUnspentWitnessDisabled(
		minConfs, maxConfs, DefaultAccountName,
	)
}

/*obd update wxf
omni core not support ListUnspentWitnessFromDefaultAccount
*/
func (l *LightningWallet) ListUnspentP2pkhFromDefaultAccount(
	minConfs, maxConfs int32) ([]*Utxo, error) {

	return l.WalletController.ListUnspentP2pkh(
		minConfs, maxConfs, DefaultAccountName,
	)
}
...
func (l *LightningWallet) handleFundingReserveRequest(req *InitFundingReserveMsg) {
	...
	if req.ChanFunder == nil {
		...
		cfg := chanfunding.WalletConfig{
			...
			CoinLocker:       l,
			Signer:           l.Cfg.Signer,
			DustLimit:        DustLimitForSize(input.P2PKHSize),
		}
	...
	if !ok {
		...
		fundingReq := &chanfunding.Request{
			...
			ChangeAddr: func() (btcutil.Address, error) {
				return l.OB_NewAddress(
					//WitnessPubKey, true, DefaultAccountName,
					PubKey, true, DefaultAccountName,
				)
			},
		}
...
func (l *LightningWallet) handleFundingCounterPartySigs(msg *addCounterPartySigsMsg) {
	...
	//sigHash, err := txscript.SignatureScript(
	//	witnessScript, hashCache, txscript.SigHashAll, commitTx,
	//	0, channelValue,
	//)
	sigHash, err := txscript.CalcSignatureHash(witnessScript, txscript.SigHashAll, commitTx, 0)
	if err != nil {
		msg.err <- err
		msg.completeChan <- nil
		return
...
func (l *LightningWallet) handleSingleFunderSigs(req *addSingleFunderSigsMsg) {
	...
	witnessScript, fundOut, err := input.GenFundingPkScript(
		ourKey.PubKey.SerializeCompressed(),
	...
	//	channelValue,
	//)
	sigHash, err := txscript.CalcSignatureHash(witnessScript, txscript.SigHashAll, ourCommitTx, 0)
	if err != nil {
		req.err <- err
		req.completeChan <- nil
		return
	...
	//p2wsh, err := input.WitnessScriptHashDisabled(witnessScript)
	//if err != nil {
	//	req.err <- err
	//	req.completeChan <- nil
	//	return
...
func (l *LightningWallet) ValidateChannel(channelState *channeldb.OpenChannel,
	...
	pkScript, err := input.GenerateP2SH(witnessScript)
	if err != nil {
		return err
	}
...
func (c *CoinSource) ListCoins(minConfs int32,
	...
	/*obd update wxf
	omni not support ListUnspentWitnessFromDefaultAccount*/
	utxos, err := c.wallet.ListUnspentP2pkhFromDefaultAccount(
		minConfs, maxConfs,
	)
	if err != nil {
		return nil, err
