...
func (lc *LightningChannel) createSignDesc() error {
	...
	fundingPkScript, err := input.GenerateP2SH(multiSigScript)
	if err != nil {
		return err
	}
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	if predictOurAdd != nil && predictOurAdd.AssetAmount > 0 && predictOurAdd.AssetID == lnwire.BtcAssetId {
		return fmt.Errorf("LightningChannel validateCommitmentSanity predictOurAdd err: you may miss AssetID")
	}
	/*
		obd add wxf
		todo check Asset
	*/
	...
	if predictTheirAdd != nil && predictTheirAdd.AssetAmount > 0 && predictTheirAdd.AssetID == lnwire.BtcAssetId {
		return fmt.Errorf("LightningChannel validateCommitmentSanity predictTheirAdd err: must set AssetID biggern  than BtcAssetId")
	}
	...
	for index := range localCommitmentView.txn.TxOut {
		...
		case localCommitmentView.incomingHTLCIndex[outputIndex] != nil:
			...
			sigHash = func() ([]byte, error) {
				...
				//)
				sigHash, err := txscript.CalcSignatureHash(htlc.ourWitnessScript, txscript.SigHashAll, successTx, 0)
				if err != nil {
					return nil, err
				}
		...
		case localCommitmentView.outgoingHTLCIndex[outputIndex] != nil:
			...
			sigHash = func() ([]byte, error) {
				...
				//	int64(htlc.BtcAmount.ToSatoshis()),
				//)
				sigHash, err := txscript.CalcSignatureHash(htlc.ourWitnessScript, txscript.SigHashAll, timeoutTx, 0)
				if err != nil {
					return nil, err
				}
...
func (lc *LightningChannel) ReceiveNewCommitment(commitSig lnwire.Sig,
	...
	/*obd update wxf
	HtlcCommitment.sigHash should not use WitnessSigHash
	*/
	...
	//)
	sigHash, err := txscript.CalcSignatureHash(multiSigScript, lc.signDesc.HashType, localCommitTx, 0)
	if err != nil {
...
func (lc *LightningChannel) validateAddHtlc(pd *PaymentDescriptor) error {
	...
	if pd.AssetAmount > 0 && pd.AssetID == lnwire.BtcAssetId {
		return fmt.Errorf("LightningChannel validateAddHtlc err: must AssetID >0")
	}
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	/*obd update wxf
	use SignatureScript instead of Witness  */
	commitTx.TxIn[0].SignatureScript, err = input.SpendMultiSigWithoutW(
		lc.signDesc.WitnessScript, ourKey,
		ourSig, theirKey, theirSig,
	)
	return commitTx, err
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee btcutil.Amount,
	...
	//lc.log.Infof("CreateCloseProposal: ourBalance  %v theirBalance %v ,fee %v commit-fee %v, LocalBtcBalance %v RemoteBtcBalance %v", ourBalance, theirBalance, proposedFee, lc.channelState.LocalCommitment.CommitFee, lc.channelState.LocalCommitment.LocalBtcBalance, lc.channelState.LocalCommitment.RemoteBtcBalance)
	if err != nil {
...
func (lc *LightningChannel) Ob_AssetCreateCloseProposal(proposedFee btcutil.Amount,
	localDeliveryScript []byte,
	remoteDeliveryScript []byte, remoteSigs []input.Signature) ([]input.Signature, bool, *chainhash.Hash,
	btcutil.Amount, error) {
	...
	if lc.status == channelClosed {
		// TODO(roasbeef): check to ensure no pending payments
		return nil, endSig, nil, 0, ErrChanClosing
	}
	...
	ourBalance, theirBalance, ourAssetBalance, theirAssetBalance, err := CoopCloseBalance(
		lc.channelState.ChanType, lc.channelState.IsInitiator,
		proposedFee*2, lc.channelState.LocalCommitment,
	)
	if err != nil {
		return nil, endSig, nil, 0, err
	}

	fundingOut := lc.signDesc.Output
	localCommit := lc.channelState.LocalCommitment
	cap := localCommit.CommitFee +
		localCommit.LocalBtcBalance.ToSatoshis() +
		localCommit.RemoteBtcBalance.ToSatoshis()

	closeTx1, closeTx2 := OB_AssetCreateCooperativeCloseTx(lc.channelState.AssetID, cap, proposedFee, fundingOut.PkScript,
		fundingTxIn(lc.channelState), lc.channelState.LocalChanCfg.DustLimit,
		lc.channelState.RemoteChanCfg.DustLimit, ourBalance, theirBalance,
		ourAssetBalance, theirAssetBalance,
		localDeliveryScript, remoteDeliveryScript,
	...
	// consensus rules such as being too big, or having any value with a
	// negative output.
	tx := btcutil.NewTx(closeTx1)
	if err := blockchain.CheckTransactionSanity(tx); err != nil {
		return nil, endSig, nil, 0, err
	}
	if len(closeTx2.TxIn) > 0 {
		tx = btcutil.NewTx(closeTx2)
		if err := blockchain.CheckTransactionSanity(tx); err != nil {
			return nil, endSig, nil, 0, err
		}
	}
	...
	// has been confirmed.
	//lc.signDesc.SigHashes = txscript.NewTxSigHashes(closeTx)
	sigs := []input.Signature{}
	sig1, err := lc.Signer.SignOutputRaw(closeTx1, lc.signDesc)
	if err != nil {
		return nil, endSig, nil, 0, err
	}
	sigs = append(sigs, sig1)
	if len(closeTx2.TxIn) > 0 && len(remoteSigs) > 0 {
		//get closeTx1.TxHash()
		ourKey := lc.channelState.LocalChanCfg.MultiSigKey.PubKey.
			SerializeCompressed()
		theirKey := lc.channelState.RemoteChanCfg.MultiSigKey.PubKey.
		...
		sigScript, err := input.SpendMultiSigWithoutW(
			lc.signDesc.WitnessScript, ourKey, sig1, theirKey,
			remoteSigs[0],
		)
		closeTx1.TxIn[0].SignatureScript = sigScript
		vm, err := txscript.NewEngine(fundingOut.PkScript, closeTx1, 0,
			txscript.StandardVerifyFlags, nil, nil, fundingOut.Value)
		if err != nil {
			return nil, endSig, nil, 0, err
		}
		...
		//set  closeTx2.PreviousOutPoint
		closeTx2.TxIn[0].PreviousOutPoint.Hash = closeTx1.TxHash()
		sig2, err := lc.Signer.SignOutputRaw(closeTx2, lc.signDesc)
		if err != nil {
			return nil, endSig, nil, 0, err
		}
	...
	closeTXID := closeTx1.TxHash()
	lc.log.Infof("Ob_AssetCreateCloseProposal: closeTXID1 %v", closeTXID)
	return sigs, endSig, &closeTXID, ourBalance, nil
}
...
func (lc *LightningChannel) OB_AssetCompleteCooperativeClose(
	localSig, remoteSig []input.Signature,
	localDeliveryScript, remoteDeliveryScript []byte,
	proposedFee btcutil.Amount) (*wire.MsgTx, *wire.MsgTx, btcutil.Amount, error) {
	...
	theirKey := lc.channelState.RemoteChanCfg.MultiSigKey.PubKey.
		SerializeCompressed()
	sigScript, err := input.SpendMultiSigWithoutW(
		lc.signDesc.WitnessScript, ourKey, localSig[0], theirKey,
		remoteSig[0],
	)
	closeTx1.TxIn[0].SignatureScript = sigScript
	vm, err := txscript.NewEngine(fundingOut.PkScript, closeTx1, 0,
	...
	if len(closeTx2.TxIn) > 0 {
		lc.log.Infof("OB_AssetCompleteCooperativeClose: closeTx1Hash %v, old closeTXID1 %v", closeTx1.TxHash(), closeTx2.TxIn[0].PreviousOutPoint.Hash)
		closeTx2.TxIn[0].PreviousOutPoint.Hash = closeTx1.TxHash()
		sigScript, err := input.SpendMultiSigWithoutW(
			lc.signDesc.WitnessScript, ourKey, localSig[1], theirKey,
			remoteSig[1],
		)
		closeTx2.TxIn[0].SignatureScript = sigScript
		vm, err := txscript.NewEngine(fundingOut.PkScript, closeTx2, 0,
			txscript.StandardVerifyFlags, nil, nil, closeTx1.TxOut[2].Value)
		if err != nil {
			return nil, nil, 0, err
		}
		if err := vm.Execute(); err != nil {
	...
	return closeTx1, closeTx2, ourBalance, nil
}
...
// transaction in full.
func CreateCooperativeCloseTx(amts *op.PksAmounts, fundingTxIn wire.TxIn,
	...
	if ourBalance >= localDust {
		...
		amts.Add(ourDeliveryScript, ourAssetBalance)
	}
	if theirBalance >= remoteDust {
		...
		amts.Add(theirDeliveryScript, theirAssetBalance)
	}
...
func OB_AssetCreateCooperativeCloseTx(assetId uint32, cap, fee btcutil.Amount, fundIngPkScript []byte, fundingTxIn wire.TxIn,
	...
	tx1Pk := ourDeliveryScript
	tx1Balace := ourBalance
	tx1AssetBalace := ourAssetBalance
	tx2Pk := theirDeliveryScript
	tx2Balace := theirBalance
	tx2AssetBalace := theirAssetBalance

	if tx1AssetBalace == 0 {
		tx1Pk = theirDeliveryScript
		tx1Balace = theirBalance
		tx1AssetBalace = theirAssetBalance
		tx2Balace = 0
		tx2AssetBalace = 0
	}
	if tx2AssetBalace == 0 {
	...
	if tx1AssetBalace > 0 && tx2AssetBalace > 0 && bytes.Compare(ourDeliveryScript, theirDeliveryScript) > 0 {
		tx1Pk = theirDeliveryScript
		tx1Balace = theirBalance
		tx1AssetBalace = theirAssetBalance
		tx2Pk = ourDeliveryScript
		tx2Balace = ourBalance
		tx2AssetBalace = ourAssetBalance
	}
	changeBalance := cap - tx1Balace - fee

	// Construct the transaction to perform a cooperative closure of the
	// channel. In the event that one side doesn't have any settled funds
	// within the channel then a refund output for that particular side can
	// be omitted.
	closeTx1 := wire.NewMsgTx(2)
	closeTx1.AddTxIn(&fundingTxIn)

	// Create both cooperative closure outputs, properly respecting the
	// dust limits of both parties.
	if tx1AssetBalace >= 0 {
		opSimple := &op.OpreturnSimple{PropertyId: assetId, Amount: uint64(tx1AssetBalace)}
		opSimple.AddOpReturnToTx(closeTx1)
		closeTx1.AddTxOut(&wire.TxOut{
			PkScript: tx1Pk,
			Value:    int64(tx1Balace),
		})
		if changeBalance > 0 {
			closeTx1.AddTxOut(&wire.TxOut{
				PkScript: fundIngPkScript,
				Value:    int64(changeBalance),
			})
		}
	}

	closeTx2 := wire.NewMsgTx(2)
	if tx2AssetBalace > 0 {
		closeTx2.AddTxIn(wire.NewTxIn(&wire.OutPoint{Hash: closeTx1.TxHash(), Index: 2}, nil, nil))

		opSimple := &op.OpreturnSimple{PropertyId: assetId, Amount: uint64(tx2AssetBalace)}
		opSimple.AddOpReturnToTx(closeTx2)

		closeTx2.AddTxOut(&wire.TxOut{
			PkScript: tx2Pk,
			Value:    int64(tx2Balace),
		})
	}

	//txsort.InPlaceSort(closeTx2)

	return closeTx1, closeTx2
}
