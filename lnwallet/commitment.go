...
func CommitScriptToSelf(chanType channeldb.ChannelType, initiator bool,
	...
	*ScriptInfo, error) {
	...
	toLocalScriptHash, err := input.GenerateP2SH(toLocalRedeemScript)
	if err != nil {
		return nil, err
	}
...
func CommitScriptToRemote(chanType channeldb.ChannelType, initiator bool,
	...
	case chanType.HasLeaseExpiration() && !initiator:
		...
		p2wsh, err := input.GenerateP2SH(script)
		if err != nil {
			return nil, 0, err
		}
	...
	default:
		...
		p2wkh, err := input.GenerateP2PKH(key.SerializeCompressed())
		if err != nil {
			return nil, 0, err
		}
...
func CommitScriptAnchors(localChanCfg,
	...
	*ScriptInfo, error) {
	...
	anchorScript := func(key *btcec.PublicKey) (*ScriptInfo, error) {
		...
		scriptHash, err := input.GenerateP2SH(script)
		if err != nil {
			return nil, err
		}
...
func genHtlcScript(chanType channeldb.ChannelType, isIncoming, ourCommit bool,
	...
	htlcP2WSH, err := input.GenerateP2SH(witnessScript)
	if err != nil {
		return nil, nil, err
	}
