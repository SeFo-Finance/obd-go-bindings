...
type WalletController interface {
	...
	*	omni wallet's balance is queryed by address
	 */
	ConfirmedBalanceByAddress(confs int32, address, accountFilter string) (btcutil.Amount, error)
	...
	OB_NewAddress(addrType AddressType, change bool,
	...
	*  obd wallet: when user invoke newadress command ,the pubkey address should record.
	 */
	RecordAddress(address string) (err error)
	OB_ListRecAddress() ([]*RecAddress, error)
	OB_SetDefaultAddress(address string) (err error)
	OB_GetDefaultAddress() (address btcutil.Address, err error)
	...
	//
	// NOTE: This method requires the global coin selection lock to be held.
	ListUnspentWitnessDisabled(minConfs, maxConfs int32,
		accountFilter string) ([]*Utxo, error)

	/*obd add wxf
	omni not support ListUnspentWitness*/
	ListUnspentP2pkh(minConfs, maxConfs int32,
		accountFilter string) ([]*Utxo, error)
