...
func (f *FullIntent) CompileFundingTx(extraInputs []*wire.TxIn,
	...
	*/
	...
	/*obd upadte wxf
	omni don't support witness-sign*/
	...
	f.signer.SignTransaction(fundingTx)
...
func (w *WalletAssembler) ProvisionChannel(r *Request) (Intent, error) {
	...
	err := w.cfg.CoinSelectLocker.WithCoinSelectLock(func() error {
		...
		/*omni only only support the first input with asset;
		todo check the  first input have enough asset
		*/
		...
		if changeAmt != 0 {
			...
			/*omni only only support the first input with asset;
			selectedCoins[0].addre may have enough asset for next openchannel, so we charge to selectedCoins[0].addre
			*/
