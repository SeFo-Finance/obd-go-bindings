...
import (
	...
	"github.com/SeFo-Finance/obd-go-bindings/lnwallet/chanfunding"
...
func (b *BtcWallet) ConfirmedBalance(confs int32,
	...
	witnessOutputs, err := b.ListUnspentP2pkh(
...
/* obd update wxf
*	omni wallet's balance is queryed by address
 */
func (b *BtcWallet) ConfirmedBalanceByAddress(confs int32,
	address, accountFilter string) (btcutil.Amount, error) {

	var balance btcutil.Amount

	//pubkeyOutputs, err := b.ListUnspentWitness(
	pubkeyOutputs, err := b.ListUnspentP2pkh(
		confs, math.MaxInt32, accountFilter,
	)
	if err != nil {
		return 0, err
	...
	for _, witnessOutput := range pubkeyOutputs {
		if address == witnessOutput.Address {
			balance += witnessOutput.Value
		}
	}
	return balance, nil
...
//
// This is a part of the WalletController interface.
func (b *BtcWallet) OB_NewAddress(t lnwallet.AddressType, change bool,
...
func (b *BtcWallet) OB_ListAddresses(addrType lnwallet.AddressType, accountName string) (items []string, err error) {
	...
	smgr, err := b.wallet.Manager.FetchScopedKeyManager(keyScope)
	if err != nil {
		return nil, err
	}
	err = walletdb.View(b.wallet.Database(), func(tx walletdb.ReadTx) error {
		ns := tx.ReadBucket([]byte("waddrmgr"))
		smgr.ForEachAccountAddress(ns, account, func(maddr waddrmgr.ManagedAddress) error {
			items = append(items, maddr.Address().String())
			return nil
		})
		return nil
	})
...
var useraddressBucketName = []byte("useraddresses")

func (b *BtcWallet) RecordAddress(address string) (err error) {
	err = walletdb.Update(b.wallet.Database(), func(tx walletdb.ReadWriteTx) error {
		first := false
		ns := tx.ReadWriteBucket(useraddressBucketName)
		if ns == nil {
			ns, err = tx.CreateTopLevelBucket(useraddressBucketName)
			if err != nil {
				return err
			}
			first = true
		}
		//set the first address  as default-addres.
		value := byte(0)
		if first {
			value = byte(1)
		}
		//prefix the key with  a sequence-number,the data will be stored in order by number
		seq, _ := ns.NextSequence()
		key := []byte(fmt.Sprintf("%4d%s", seq, address))
		err = ns.Put([]byte(key), []byte{value})
		if err != nil {
			return err
		}
		return nil
...
func (b *BtcWallet) OB_ListRecAddress() ([]*lnwallet.RecAddress, error) {
	items := []*lnwallet.RecAddress{}
	err := walletdb.View(b.wallet.Database(), func(tx walletdb.ReadTx) error {
		ns := tx.ReadBucket(useraddressBucketName)
		if ns == nil {
			return nil
		}
		err := ns.ForEach(func(k, v []byte) error {
			items = append(items, &lnwallet.RecAddress{Addre: string(k)[4:], IsDefault: bytes.Equal(v, []byte{1})})
			return nil
		})
		return err
	})
	//d, err1 := b.GetDefaultAddress()
	//fmt.Println("GetDefaultAddress", d, err1)
	return items, err
}
func (b *BtcWallet) OB_GetDefaultAddress() (address btcutil.Address, err error) {
	err = walletdb.View(b.wallet.Database(), func(tx walletdb.ReadTx) error {
		ns := tx.ReadBucket(useraddressBucketName)
		if ns == nil {
			return errors.New("wallet's DefaultAddress not set")
		}
		addrStr := ""
		err = ns.ForEach(func(k, v []byte) error {
		...
		if addrStr == "" {
			return errors.New("wallet's DefaultAddress not found")
		}
		address, err = btcutil.DecodeAddress(addrStr, b.netParams)
		return err
	})
	return
}
func (b *BtcWallet) OB_SetDefaultAddress(address string) (err error) {
	err = walletdb.Update(b.wallet.Database(), func(tx walletdb.ReadWriteTx) error {
		ns := tx.ReadWriteBucket(useraddressBucketName)
		if ns == nil {
			ns, err = tx.CreateTopLevelBucket(useraddressBucketName)
		...
		var hit = false
		err = ns.ForEach(func(k, v []byte) error {
			if strings.HasSuffix(string(k), address) && bytes.Equal([]byte{0}, v) {
				err = ns.Put(k, []byte{1})
				hit = true
			} else {
				err = ns.Put(k, []byte{0})
...
func (b *BtcWallet) CreateSimpleTxForSelectedCoins(toAddress btcutil.Address, selectedCoins []chanfunding.Coin, amt, changeAmt btcutil.Amount, assetAmt omnicore.Amount, assetId uint32, dryrun bool) (*wire.MsgTx, error) {
	...
	//add tx.out
	payToScirpt, err := txscript.PayToAddrScript(toAddress)
	if err != nil {
		return nil, err
	}
...
//
// This is a part of the WalletController interface.
func (b *BtcWallet) ListUnspentWitnessDisabled(minConfs, maxConfs int32,
...
/*omni fund not  support witness utxo*/
func (b *BtcWallet) ListUnspentP2pkh(minConfs, maxConfs int32,
	accountFilter string) ([]*lnwallet.Utxo, error) {
	...
	// Next, we'll run through all the regular outputs, only saving those
	// which are p2pkh outputs
	utxos := make([]*lnwallet.Utxo, 0, len(unspentOutputs))
	for _, output := range unspentOutputs {
		pkScript, err := hex.DecodeString(output.ScriptPubKey)
		if err != nil {
			return nil, err
		}
		sclass, addrs, _, err := txscript.ExtractPkScriptAddrs(pkScript, b.netParams)
		if err != nil {
			return nil, err
		}
