...
func (b *BtcWallet) FetchInputInfo(prevOut *wire.OutPoint) (*lnwallet.Utxo, error) {
	...
	stype := txscript.GetScriptClass(txOut.PkScript)
	if stype != txscript.PubKeyHashTy {
		return nil, errors.New("FetchInputInfo err,txOut not a PubKeyHashTy")
	}
...
func (b *BtcWallet) ComputeInputScript(tx *wire.MsgTx,
	...
	return nil, errors.New("ComputeInputScript is for witness, omni  diabled it")
...
func (b *BtcWallet) ComputePkhInputScript(tx *wire.MsgTx, signDesc *input.SignDescriptor) (*input.Script,
	error) {
	privKey, err := b.fetchPrivKey(&signDesc.KeyDesc)
	if err != nil {
		return nil, err
	...
	sig, err := txscript.RawTxInSignature(tx, signDesc.InputIndex, signDesc.Output.PkScript, signDesc.HashType, privKey)
	if err != nil {
		return nil, err
	}
...
func (b *BtcWallet) SignTransaction(tx *wire.MsgTx) error {
	sigErrs, err := b.wallet.SignTransaction(tx, txscript.SigHashAll, nil, nil, nil)
	if err != nil || len(sigErrs) > 0 {
		estr := ""
		for _, sigErr := range sigErrs {
			estr += sigErr.Error.Error() + " "
		}
		if err != nil {
			estr += err.Error()
		}
