...
func testInvoiceWorkflow(t *testing.T, test invWorkflowTest) {
	...
	_, err = db.UpdateInvoice(ref, nil, getUpdateInvoice(lnwire.MilliSatoshi(payAmt)))
	if err != nil {
...
func TestInvoiceCancelSingleHtlc(t *testing.T) {
	...
	testInvoice := &Invoice{
		Htlcs: map[CircuitKey]*InvoiceHTLC{},
		Terms: ContractTerm{
			Value:           lnwire.UnitPrec11(lnwire.NewMSatFromSatoshis(10000)),
			Features:        emptyFeatures,
...
func TestInvoiceCancelSingleHtlcAMP(t *testing.T) {
	...
	invoice.AmtPaid = lnwire.UnitPrec11(2 * amt)
	...
	invoice.AMPState[*setID2] = InvoiceStateAMP{
		...
		AmtPaid: lnwire.UnitPrec11(amt * 2),
		InvoiceKeys: map[CircuitKey]struct{}{
			{HtlcID: 1}: {},
			{HtlcID: 2}: {},
		},
	...
	invoice.AmtPaid = lnwire.UnitPrec11(amt)

	ampState := invoice.AMPState[*setID2]
	ampState.State = HtlcStateCanceled
	ampState.AmtPaid = lnwire.UnitPrec11(amt)
...
func TestInvoiceAddTimeSeries(t *testing.T) {
	...
	for i := 10; i < len(invoices); i++ {
		...
		ref := InvoiceRefByHash(paymentHash)
		_, err := db.UpdateInvoice(
			ref, nil, getUpdateInvoice(lnwire.MilliSatoshi(invoice.Terms.Value)),
		)
		if err != nil {
			t.Fatalf("unable to settle invoice: %v", err)
		}
...
func TestSetIDIndex(t *testing.T) {
	...
	invoice.AmtPaid = lnwire.UnitPrec11(amt * 3)
...
func TestUnexpectedInvoicePreimage(t *testing.T) {
	...
	_, err = db.UpdateInvoice(
		...
		getUpdateInvoice(lnwire.MilliSatoshi(invoice.Terms.Value)),
	)
