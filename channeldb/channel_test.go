...
func channelCommitmentOption(height uint64, localBalance,
	...
	return func(params *testChannelParams) {
		if local {
			...
			params.channel.LocalCommitment.LocalBtcBalance = localBalance
			params.channel.LocalCommitment.RemoteBtcBalance = remoteBalance
		} else {
			params.channel.RemoteCommitment.CommitHeight = height
			params.channel.RemoteCommitment.LocalBtcBalance = localBalance
			params.channel.RemoteCommitment.RemoteBtcBalance = remoteBalance
		}
	}
}
...
func createTestChannelState(t *testing.T, cdb *ChannelStateDB) *OpenChannel {
	...
	localCfg := ChannelConfig{
		ChannelConstraints: ChannelConstraints{
			...
			MaxPendingAmount: lnwire.UnitPrec11(lnwire.MilliSatoshi(rand.Int63())),
			ChanReserve:      lnwire.UnitPrec8(btcutil.Amount(rand.Int63())),
			MinHTLC:          lnwire.UnitPrec11(lnwire.MilliSatoshi(rand.Int63())),
	...
	return &OpenChannel{
		...
		IsPending:         true,
		IdentityPub:       pubKey,
		BtcCapacity:       btcutil.Amount(10000),
		...
		LocalCommitment: ChannelCommitment{
			CommitHeight:     0,
			LocalBtcBalance:  lnwire.MilliSatoshi(9000),
			RemoteBtcBalance: lnwire.MilliSatoshi(3000),
		...
		RemoteCommitment: ChannelCommitment{
			CommitHeight:     0,
			LocalBtcBalance:  lnwire.MilliSatoshi(3000),
			RemoteBtcBalance: lnwire.MilliSatoshi(9000),
			CommitFee:        btcutil.Amount(rand.Int63()),
			FeePerKw:         btcutil.Amount(5000),
			CommitTx:         channels.TestFundingTx,
			CommitSig:        bytes.Repeat([]byte{1}, 71),
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	closeSummary := &ChannelCloseSummary{
		...
		SettledBalance:    lnwire.UnitPrec8(btcutil.Amount(500)),
		TimeLockedBalance: lnwire.UnitPrec8(btcutil.Amount(10000)),
		IsPending:         false,
...
func TestChannelStateTransition(t *testing.T) {
	...
	commitment := ChannelCommitment{
		...
		RemoteLogIndex:   2,
		RemoteHtlcIndex:  1,
		LocalBtcBalance:  lnwire.MilliSatoshi(1e8),
		RemoteBtcBalance: lnwire.MilliSatoshi(1e8),
		CommitFee:        55,
		FeePerKw:         99,
		CommitTx:         newTx,
		CommitSig:        newSig,
	...
	remoteCommit.LocalBtcBalance = lnwire.MilliSatoshi(2e8)
	remoteCommit.RemoteBtcBalance = lnwire.MilliSatoshi(3e8)
		...
		LogUpdates: []LogUpdate{
			{
				...
				UpdateMsg: &lnwire.UpdateAddHTLC{
					ID:        1,
					Amount:    lnwire.UnitPrec11(lnwire.NewMSatFromSatoshis(100)),
					Expiry:    25,
					ExtraData: make([]byte, 0),
				},
			},
			{
				...
				UpdateMsg: &lnwire.UpdateAddHTLC{
					ID:        2,
					Amount:    lnwire.UnitPrec11(lnwire.NewMSatFromSatoshis(200)),
					Expiry:    50,
					ExtraData: make([]byte, 0),
				},
			},
	...
	commitDiff.Commitment.LocalBtcBalance -= htlcAmt
	commitDiff.Commitment.RemoteBtcBalance += htlcAmt
...
func TestFetchClosedChannels(t *testing.T) {
	...
	summary := &ChannelCloseSummary{
		...
		SettledBalance:    lnwire.UnitPrec8(state.LocalCommitment.LocalBtcBalance.ToSatoshis()),
		TimeLockedBalance: lnwire.UnitPrec8(state.RemoteCommitment.LocalBtcBalance.ToSatoshis() + 10000),
		CloseType:         RemoteForceClose,
		IsPending:         true,
...
func TestBalanceAtHeight(t *testing.T) {
	...
	putRevokedState := func(c *OpenChannel, height uint64, local,
		...
		err := kvdb.Update(c.Db.backend, func(tx kvdb.RwTx) error {
			...
			commit.LocalBtcBalance = local
			commit.RemoteBtcBalance = remote
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			local, remote, _, _, err := channel.BalancesAtHeight(
				test.targetHeight,
			)
