...
func TestEdgeInfoUpdates(t *testing.T) {
	...
	require.Len(t, graph.graphCache.assetNodeChannels, 0)
...
func assertNodeNotInCache(t *testing.T, g *ChannelGraph, n route.Vertex) {
	...
	require.False(t, ok)
	//TODO:assetID
	_, ok = g.graphCache.assetNodeChannels[1][n]
	require.False(t, ok)
...
func assertEdgeWithNoPoliciesInCache(t *testing.T, g *ChannelGraph,
	...
	// Let's check the internal view first.
	//TODO:assetId
	require.NotEmpty(t, g.graphCache.assetNodeChannels[1][e.NodeKey1Bytes])
	require.NotEmpty(t, g.graphCache.assetNodeChannels[1][e.NodeKey2Bytes])
	...
	//TODO:assetID
	require.Contains(
		t, g.graphCache.assetNodeChannels[1][e.NodeKey1Bytes], e.ChannelID,
	)
	//TODO:assetID
	require.Equal(
		t, expectedNode1Channel,
		g.graphCache.assetNodeChannels[1][e.NodeKey1Bytes][e.ChannelID],
	)
	...
	//TODO:assetId
	require.Contains(
		t, g.graphCache.assetNodeChannels[1][e.NodeKey2Bytes], e.ChannelID,
	)
	//TODO:assetId
	require.Equal(
		t, expectedNode2Channel,
		g.graphCache.assetNodeChannels[1][e.NodeKey2Bytes][e.ChannelID],
	)

	// The external view should reflect this as well.
	var foundChannel *DirectedChannel
	//TODO:assetId
	err := g.graphCache.ForEachChannel(1,
...
func assertNoEdge(t *testing.T, g *ChannelGraph, chanID uint64) {
	...
	//TODO:assetId
	for _, channels := range g.graphCache.assetNodeChannels[1] {
...
func assertEdgeWithPolicyInCache(t *testing.T, g *ChannelGraph,
	...
	//TODO:assetId
	c1, ok := g.graphCache.assetNodeChannels[1][e.NodeKey1Bytes][e.ChannelID]
	require.True(t, ok)
	...
	//TODO:assetId
	c2, ok := g.graphCache.assetNodeChannels[1][e.NodeKey2Bytes][e.ChannelID]
	require.True(t, ok)
	...
	//TODO:assetId
	require.NoError(t, g.graphCache.ForEachChannel(1,
		e.NodeKey1Bytes, func(c *DirectedChannel) error {
			c1Ext = c
...
func newEdgePolicy(chanID uint64, db kvdb.Backend,
	...
	return &ChannelEdgePolicy{
		...
		MinHTLC:                   lnwire.UnitPrec11(lnwire.MilliSatoshi(prand.Int63())),
		MaxHTLC:                   lnwire.UnitPrec11(lnwire.MilliSatoshi(prand.Int63())),
		FeeBaseMSat:               lnwire.MilliSatoshi(prand.Int63()),
		FeeProportionalMillionths: lnwire.UnitPrec11(lnwire.MilliSatoshi(prand.Int63())),
		db:                        db,
	}
}
...
func TestGraphTraversal(t *testing.T) {
	...
	//TODO:assetId
	err = graph.ForEachNodeCached(1, func(node route.Vertex,
...
func TestGraphCacheTraversal(t *testing.T) {
	...
	for _, node := range nodeList {
		node := node
		//TODO:assetId
		err = graph.graphCache.ForEachChannel(1,
...
func TestComputeFee(t *testing.T) {
	...
	fee := policy.ComputeFee(lnwire.UnitPrec11(outgoingAmt))
	if fee != lnwire.UnitPrec11(expectedFee) {
		t.Fatalf("expected fee %v, got %v", expectedFee, fee)
	}

	fwdFee := policy.ComputeFeeFromIncoming(lnwire.UnitPrec11(outgoingAmt) + fee)
	if fwdFee != lnwire.UnitPrec11(expectedFee) {
			...
			for _, n := range nodes {
				err := n.ForEachChannel(
					tx, func(tx kvdb.RTx,
						...
						// results.
						totalCapacity += btcutil.Amount(info.Capacity)
						maxHTLCs += lnwire.MilliSatoshi(policy.MaxHTLC)
						maxHTLCs += lnwire.MilliSatoshi(policy2.MaxHTLC)
...
func TestGraphCacheForEachNodeChannel(t *testing.T) {
	...
	var numChans int
	//TODO:assetID
	err = graph.ForEachNodeChannel(1, nil, node1.PubKeyBytes,
...
func TestGraphLoading(t *testing.T) {
	...
	//TODO:assetID
	require.Equal(
		t, graph.graphCache.assetNodeChannels[1],
		graphReloaded.graphCache.assetNodeChannels[1],
	)
