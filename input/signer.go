...
type Signer interface {
	...
	ComputeInputScript(tx *wire.MsgTx, signDesc *SignDescriptor) (*Script,
		error)
	ComputePkhInputScript(tx *wire.MsgTx, signDesc *SignDescriptor) (*Script,
		error)

	//SignTransaction sign p2kh.input
...
func (s *Script) GetSigScript() ([]byte, error) {
	...
	for _, witness := range s.Witness {
		if witness != nil {
			sBuild.AddData(witness)
		} else {
			sBuild.AddOp(txscript.OP_FALSE)
		}
	}
	return sBuild.Script()
}
